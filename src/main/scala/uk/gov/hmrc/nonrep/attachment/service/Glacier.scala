          ...
          eventuallyArchive(attachmentContent, datedVaultName(attachmentContent.info.notableEvent)).map {
            (archiveIdOrError: EitherErr[String]) =>
              archiveIdOrError.map { archiveId =>
                ArchivedAttachment(
                  attachmentContent.info.copy(attachmentSize = Some(attachmentContent.length)),
                  archiveId,
                  datedVaultName(attachmentContent.info.notableEvent)
                )
              }
          }
        case Left(e)                  =>
...
object ChecksumUtils {
  ...
  def sha256TreeHashHex(payload: Array[Byte]): String =
    if payload.isEmpty then "" else toHex(sha256TreeHash(splitPayloadIntoChunksAndSha256HashEachChunk(payload)))
  ...
  private def sha256TreeHash(sha256Hashes: Seq[Array[Byte]]): Array[Byte] =
     ...
     */
    if sha256Hashes.tail.isEmpty then
       ...
       */
      val concatenatedPairsOfSha256Hashes = sha256Hashes
        .grouped(2)
        .map {
  ...
  private def toHex(data: Array[Byte]) = {
    val stringBuilder = StringBuilder(data.length * 2)

    data.indices.foreach { index =>
      val hex = toHexString(data(index) & 0xff)
      if hex.length == 1 then stringBuilder.append("0")
