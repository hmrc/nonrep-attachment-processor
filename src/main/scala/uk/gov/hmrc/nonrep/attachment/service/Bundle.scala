...
class BundleService()(using config: ServiceConfig) extends Bundle {
      ...
      _.flatMap(content =>
        Using
          .Manager { use =>
            ...
            content.files.foreach { case (name, file) =>
          ...
          .flatMap(exception =>
            Left(
              ErrorMessage(s"Failure of creating zip archive for ${content.info.s3ObjectKey} with ${exception.getCause}", Some(exception))
            )
          )
      ).flatten
    }
  ...
  private def handleFileExtraction(
    ...
    responseF(filename).left
      .map(e =>
        ErrorMessage(
          s"Failure of extracting $filename from zip archive for attachment ${attachment.s3ObjectKey} relating to ${attachment.submissionId.getOrElse("INVALID")} with ${e.getCause}",
          Some(e),
          ERROR
        )
      )
      .flatMap(
        _.toRight(
          ErrorMessage(
            s"ADMIN_REQUIRED: Failure of extracting zip archive for ${attachment.s3ObjectKey} with file $filename not found",
            None,
            ERROR
          )
        )
